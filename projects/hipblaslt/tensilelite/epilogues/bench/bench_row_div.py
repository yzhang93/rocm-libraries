...
_KERNEL_DIR  = os.path.join(os.path.dirname(os.path.dirname(os.path.abspath(__file__))),
                             "kernels")
...
def make_verify_fn(c_orig, d_host, inv_d, eps):
    ...
    """
    row_sums = d_host.astype(np.float64).sum(axis=1)
...
def benchmark(chip, m, n, n_d, inv_d, eps, block_size, warmup, num_iters, do_verify):
    ...
    # Kernarg layout (offsets match .amdhsa_kernarg_size 64):
    #   0:  C ptr (8B)     8:  D ptr (8B)     16: padding i32 (unused)
    #   20: n (int32)      24: n_c (int32)     28: n_d (int32)
    #   32: inv_d (f32)    36: eps (f32)
    args = [
        InOutArray(c_host),
        InputArray(d_host),
        np.int32(0),          # offset 16: padding (unused by kernel)
        np.int32(n),          # offset 20: total columns
        np.int32(block_size), # offset 24: columns per block (n_c)
        np.int32(n_d),        # offset 28: number of partial-sum tiles
        np.float32(inv_d),    # offset 32: inv_d
        np.float32(eps),      # offset 36: eps
    ]
