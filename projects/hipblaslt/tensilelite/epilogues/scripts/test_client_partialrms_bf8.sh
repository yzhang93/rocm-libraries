...
#
# Integration test for the tensilelite-client --use-partial-rms path with bfp8 (OCP e5m2) A/B input.
#
# Steps:
...
#
# Usage:
#   epilogues/scripts/test_client_partialrms_bf8.sh [--chip CHIP] [--client PATH] [--out-dir DIR]
#
# Arguments:
#   --chip   GPU architecture string (default: gfx950).
#   --client Path to the tensilelite-client binary (required if not on PATH).
#   --out-dir Scratch directory for generated files (default: /tmp/prms_bf8_test_<chip>).
#
# Prerequisites:
...
if [[ -z "$OUT_DIR" ]]; then
    OUT_DIR="/tmp/prms_bf8_test_${CHIP}"
fi
...
python3 "${TENSILELITE_ROOT}/Tensile/bin/Tensile" \
    "${TENSILELITE_ROOT}/epilogues/yaml/gemm_partial_rms_k1_rowmajor_bf8.yaml" \
    "${OUT_DIR}/tensile_out"
...
# A/B are OCP bfp8 e5m2; C/D remain bf16 (PartialRMS gamma output is bf16).
COMMON_ARGS=(
    --library-file      "$LIB_YAML"
    --code-object       "$LIB_CO"
    --problem-identifier "Contraction_l_Alik_Bljk_Cijk_Dijk"
    --a-type BFloat8 --b-type BFloat8 --c-type BFloat16 --d-type BFloat16
    --compute-input-type-A BFloat8 --compute-input-type-B BFloat8
