...
#
# Integration test for the tensilelite-client --dquant-type mxfp8 path.
#
# Steps:
...
#   3. Run the client for several (M, N, K) shapes and MX block sizes,
#      verifying the client reference checker reports PASSED for each.
#
# Usage:
#   epilogues/scripts/test_client_mxfp8quant.sh [--chip CHIP] [--client PATH] [--out-dir DIR]
#
# Arguments:
#   --chip    GPU architecture string (default: gfx950).
#   --client  Path to the tensilelite-client binary (required if not on PATH).
#   --out-dir Scratch directory for generated files (default: /tmp/mx_test_<chip>).
#
# Prerequisites:
...
python3 "${TENSILELITE_ROOT}/Tensile/bin/Tensile" \
    "${TENSILELITE_ROOT}/epilogues/yaml/gemm_mxfp8_quant_k1.yaml" \
    "${OUT_DIR}/tensile_out"
TENSILE_EXIT=$?
set -e
...
# --dquant-type mxfp8 enables the epilogue; the client reference validates both
# MXScale (e8m0 bytes, exact) and D (fp8) against the CPU reference.
COMMON_ARGS=(
