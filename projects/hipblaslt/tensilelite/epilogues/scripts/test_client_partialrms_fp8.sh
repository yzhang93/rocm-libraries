...
#
# Integration test for the tensilelite-client --use-partial-rms path with fp8 (OCP e4m3) A/B input.
#
# Steps:
...
#
# Usage:
#   epilogues/scripts/test_client_partialrms_fp8.sh [--chip CHIP] [--client PATH] [--out-dir DIR]
#
# Arguments:
#   --chip   GPU architecture string (default: gfx950).
#   --client Path to the tensilelite-client binary (required if not on PATH).
#   --out-dir Scratch directory for generated files (default: /tmp/prms_fp8_test_<chip>).
#
# Prerequisites:
...
if [[ -z "$OUT_DIR" ]]; then
    OUT_DIR="/tmp/prms_fp8_test_${CHIP}"
fi
...
python3 "${TENSILELITE_ROOT}/Tensile/bin/Tensile" \
    "${TENSILELITE_ROOT}/epilogues/yaml/gemm_partial_rms_k1_rowmajor_fp8.yaml" \
    "${OUT_DIR}/tensile_out"
...
# A/B are OCP fp8 e4m3; C/D remain bf16 (PartialRMS gamma output is bf16).
COMMON_ARGS=(
