...
#
# Integration test for the tensilelite-client --use-partial-rms path with f16 A/B input.
#
# Steps:
...
#
# Usage:
#   epilogues/scripts/test_client_partialrms_f16.sh [--chip CHIP] [--client PATH] [--out-dir DIR]
#
# Arguments:
#   --chip   GPU architecture string (default: gfx950).
#   --client Path to the tensilelite-client binary (required if not on PATH).
#   --out-dir Scratch directory for generated files (default: /tmp/prms_f16_test_<chip>).
#
# Prerequisites:
...
if [[ -z "$OUT_DIR" ]]; then
    OUT_DIR="/tmp/prms_f16_test_${CHIP}"
fi
...
python3 "${TENSILELITE_ROOT}/Tensile/bin/Tensile" \
    "${TENSILELITE_ROOT}/epilogues/yaml/gemm_partial_rms_k1_rowmajor_f16.yaml" \
    "${OUT_DIR}/tensile_out"
...
COMMON_ARGS=(
    ...
    --a-type Half --b-type Half --c-type Half --d-type Half
    --compute-input-type-A Half --compute-input-type-B Half
