...
def buildK1Kernel(chip, wgN=2, yamlPath=None):
    """Build the K1 (PartialRMS) kernel: setup → YAML → asm → compile.

    Returns (solution, hsaco, kernelName). yamlPath=None uses the
    buildK1SolutionFromYaml default YAML.
    """
    from epilogues.tensilelite.yaml_solution_builder import buildK1SolutionFromYaml
    assembler, isaInfoMap, debugConfig = setup_tensile(chip)
    kwargs = {} if yamlPath is None else {"yamlPath": yamlPath}
    solution = buildK1SolutionFromYaml(assembler, isaInfoMap, debugConfig, wgN=wgN, **kwargs)
    asmStr, kernelName = generate_asm(solution, assembler, debugConfig)
    hsaco = amdgpu_exec.compile_asm_to_hsaco(asmStr, chip)
    return solution, hsaco, kernelName
