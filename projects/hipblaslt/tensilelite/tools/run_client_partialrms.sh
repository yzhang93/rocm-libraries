...
#
# This wraps the integration flow documented in tools/TUNING_PIPELINE.md:
#   1. Optionally build the client (invoke build-client).
#   2. Optionally generate library logic (gen_partialrms_logic.py).
#   3. Optionally compile the device library (TensileCreateLibrary).
#   4. Run the client for each test case and report PASSED / FAILED.
#
# Usage:
#   tools/run_client_partialrms.sh [OPTIONS]
#
# Options:
...
# ── Generate library logic if not provided ────────────────────────────────────
if [[ -z "$LIB_YAML" ]]; then
    echo "==> Generating PartialRMS logic YAML for $CHIP ..."
    mkdir -p "$LOGIC_DIR"
    python3 "${SCRIPT_DIR}/gen_partialrms_logic.py" \
        --chip "$CHIP" \
        --out-dir "$LOGIC_DIR"

    EXPECTED_YAML="${LOGIC_DIR}/${CHIP}/Equality/PartialRMS_BF16_TN.yaml"
    if [[ ! -f "$EXPECTED_YAML" ]]; then
        echo "error: logic YAML not generated at expected path: $EXPECTED_YAML" >&2
        exit 1
    fi
