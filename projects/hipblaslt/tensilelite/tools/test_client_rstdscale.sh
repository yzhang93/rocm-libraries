...
#
# Integration test for the tensilelite-client --use-rstd-scale path.
#
# Steps:
#   1. Generate an RstdScale LibraryLogic YAML for the target chip.
#   2. Compile a device library (YAML format) with TensileCreateLibrary.
#   3. Run the client for each test shape, verify PASSED.
#
# Usage:
#   tools/test_client_rstdscale.sh [--chip CHIP] [--client PATH] [--out-dir DIR]
#
# Arguments:
#   --chip    GPU architecture string (default: gfx950).
#   --client  Path to the tensilelite-client binary (required if not on PATH).
#   --out-dir Scratch directory for generated files (default: /tmp/rstd_test_<chip>).
#
# Prerequisites:
...
# ── Step 1: Generate LibraryLogic YAML ───────────────────────────────────────
echo "==> Generating RstdScale logic YAML for $CHIP ..."
TENSILELITE_ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
python3 "${TENSILELITE_ROOT}/tools/gen_rstdscale_logic.py" \
    --chip "$CHIP" \
    --out-dir "$LOGIC_DIR"

LOGIC_YAML="${LOGIC_DIR}/${CHIP}/Equality/RstdScale_BF16_TN.yaml"
if [[ ! -f "$LOGIC_YAML" ]]; then
    echo "error: logic YAML not generated: $LOGIC_YAML" >&2
    exit 1
fi
echo "  Logic: $LOGIC_YAML"
